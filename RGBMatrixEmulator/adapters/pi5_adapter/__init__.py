    ...
    def load_emulator_window(self):
        ...
        if sequence == "BGR":
            ...
            if hasattr(piomatter.Pinout, bgr_pinout_str):
                ...
                sequence = "RGB" # Treat as normal for rest of pipeline
                 ...
                 Logger.warning(f"BGR Sequence requested but pinout '{bgr_pinout_str}' not found in piomatter.Pinout. Will attempt software pixel swap.")

        # Resolve Pinout Enum
        if hasattr(piomatter.Pinout, pinout_str):
             pinout = getattr(piomatter.Pinout, pinout_str)
        else:
            Logger.warning(f"Pinout '{pinout_str}' not found in piomatter.Pinout. Defaulting to AdafruitMatrixBonnet.")
        ...
        if sequence != "RGB":
             ...
             if sequence in swizzle_map:
                 ...
                 Logger.info(f"Using software pixel swizzle for sequence '{sequence}': {self.pixel_swizzle}")
             else:
                 Logger.warning(f"Unknown LED sequence '{sequence}'. Display colors may be incorrect.")
        ...
        if hasattr(piomatter.Orientation, rotation_str):
            rotation = getattr(piomatter.Orientation, rotation_str)
             ...
             Logger.warning(f"Rotation '{rotation_str}' not found in piomatter.Orientation. Defaulting to Normal.")
             rotation = piomatter.Orientation.Normal

        if config.n_planes == 10 and config.n_temporal_planes == 4 and config.n_lanes == 1:
            # Simple Geometry
             ...
             if config.n_lanes > 1 or "Active3" in pinout_str:
                 try:
                     ...
                     pixelmap = simple_multilane_mapper(self.width, self.height, config.n_addr_lines, config.n_lanes)
                 except ImportError:
                     Logger.warning("Could not import simple_multilane_mapper. Active3/Multilane support may fail.")
        ...
        self.framebuffer = np.zeros(shape=(geometry.height, geometry.width, 3), dtype=np.uint8)
